...
Supports secsie and ini formats. Not suitable for writing .ini files, but reads them very well.
"""
...
MODES = {
    ...
    "ini": dict(
        ...
        FLOAT_EX = re.compile(r'^([-]?\d+[\.]\d*)$'),
        FALSE_EX = re.compile(r'(false|no|off)', re.IGNORECASE),
        NULL_EX = re.compile(r'null', re.IGNORECASE),
        TRUE_EX = re.compile(r'(true|yes|on)', re.IGNORECASE),
        INT_EX = re.compile(r'^[-]?\d+$')
    )
}
...
def generate_config(obj: dict, output_file: str = None) -> str:
    ...
    This WILL NOT write .ini files it has parsed, so don't even try. The only output format is secsie."""
